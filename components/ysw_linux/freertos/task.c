...
static int ysw_millis_to_rtos_ticks(int millis)
{
    if (millis && (millis < portTICK_PERIOD_MS)) {
        ESP_LOGW(TAG, "ysw_millis_to_rtos_ticks millis=%d < portTICK_PERIOD_MS=%d", millis, portTICK_PERIOD_MS);
    }
...
{
    return ysw_millis_to_rtos_ticks(ysw_system_get_reference_time_in_millis());
}

void vTaskDelay(int ticks)
{
    usleep(ysw_rtos_ticks_to_millis(ticks) * 1000);
}

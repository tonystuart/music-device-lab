...
static void on_sequencer_status(ysw_hpc_t *hpc, ysw_seq_status_message_t *message)
{
    if (message->type == YSW_SEQ_IDLE) {
        ysw_hpe_on_metro(hpc->hpe, NULL);
    } else if (message->type == YSW_SEQ_NOTE) {
        ysw_hpe_on_metro(hpc->hpe, message->note);
    }
}
...
static void on_stop(ysw_hpc_t *hpc, lv_obj_t *btn)
{
    stop();
}

static void on_loop(ysw_hpc_t *hpc, lv_obj_t *btn)
{
    ysw_seq_message_t message = {
        .type = YSW_SEQ_LOOP,
...
static void on_paste(ysw_hpc_t *hpc, lv_obj_t *btn)
{
    ...
    if (hpc->clipboard) {
        ...
        if (paste_count) {
            ...
            ysw_hpe_ensure_visible(hpc->hpe, first, first + paste_count - 1); // -1 because width is included
...
static ysw_frame_t *create_frame(ysw_hpc_t *hpc)
{
...
ysw_hpc_t *ysw_hpc_create(ysw_music_t *music, uint32_t hp_index)
{
    ...
    hpc->hpe = ysw_hpe_create(hpc->frame->win, hpc);
    ysw_hpe_set_create_cb(hpc->hpe, on_create_ps);
    ysw_hpe_set_edit_cb(hpc->hpe, on_edit_ps);
    ysw_hpe_set_select_cb(hpc->hpe, on_select);
    ysw_hpe_set_drag_end_cb(hpc->hpe, on_drag_end);

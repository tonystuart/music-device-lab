...
void ysw_chooser_sort(ysw_chooser_t *chooser, ysw_chooser_sort_t type)
{
    ...
    ysw_chooser_hide_selection(chooser);
    zm_section_x section_x = ysw_chooser_update_sections(chooser, section);
    ysw_chooser_show_selection(chooser, section_x);
}

ysw_chooser_t *ysw_chooser_create(zm_music_t *music, zm_section_t *section)
{
    assert(music);
    assert(section);
    ...
    zm_section_x new_row = ysw_chooser_update_sections(chooser, section);
    if (new_row != -1) {

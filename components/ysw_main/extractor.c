...
static void extract_sample(char *ofn, FILE *csvf, fluid_sample_t *sample, int sx)
{
    ...
    fprintf(csvf, "1,%d,%s,%d,%d,60,0\n", sx, sn, loop_start, loop_length);
...
static void extract_program(FILE *csvf, fluid_defpreset_t *dp, int px)
{
    char preset_name[64];
...
static void extract_samples(char *ofn, FILE *csvf, fluid_inst_t *inst, int sx)
{
    ...
    while (iz) {
        ...
        extract_sample(ofn, csvf, sample, sx);
        iz = fluid_inst_zone_next(iz);
        sx++;
    }
}
...
{
    fprintf(csvf, "#type(1=sample),index,name,reppnt,replen,volume,pan\n");
    fprintf(csvf, "#type(2=program),index,name,type,gm\n");
    fprintf(csvf, "#type(3=patch),from-note,to-note,from-vel,to-vel,sample[,name]\n");
}

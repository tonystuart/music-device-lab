...
{
    ysw_bus_subscribe(shell->bus, YSW_ORIGIN_SOFTKEY, shell->queue);
}
...
static void process_event(void *context, ysw_event_t *event)
{
    ...
    switch (event->header.type) {
        case YSW_EVENT_SOFTKEY_DOWN:
            ysw_menu_on_softkey_down(shell->menu, event);
            break;
        case YSW_EVENT_SOFTKEY_UP:
            ysw_menu_on_softkey_up(shell->menu, event);
            break;
        case YSW_EVENT_SOFTKEY_PRESSED:
            ysw_menu_on_softkey_pressed(shell->menu, event);
            break;

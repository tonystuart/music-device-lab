...
static inline void on_note_on(ysw_vs_synth_t *ysw_vs_synth, ysw_event_note_on_t *m)
{
...
static inline void on_note_off(ysw_vs_synth_t *ysw_vs_synth, ysw_event_note_off_t *m)
{
    //ESP_LOGD(TAG, "on_note_off channel=%d, note=%d", m->channel, m->midi_note);
    ysw_vs1053_set_note_off(m->channel, m->midi_note);
}

static inline void on_program_change(ysw_vs_synth_t *ysw_vs_synth, ysw_event_program_change_t *m)
{
...
{
    ysw_vs_synth_t *ysw_vs_synth = context;
    switch (event->header.type) {
        case YSW_EVENT_NOTE_ON:
            on_note_on(ysw_vs_synth, &event->note_on);
            break;
        case YSW_EVENT_NOTE_OFF:
            on_note_off(ysw_vs_synth, &event->note_off);
            break;
        case YSW_EVENT_PROGRAM_CHANGE:
            on_program_change(ysw_vs_synth, &event->program_change);
            break;
        default:
            break;
    }
...
void ysw_vs_synth_create_task(ysw_bus_t *bus, ysw_vs1053_config_t *vs1053_config)
{
    ...
    ysw_vs_synth_t *ysw_vs_synth = ysw_heap_allocate(sizeof(ysw_vs_synth_t));
    ysw_vs_synth->config = *vs1053_config;
    ysw_vs1053_initialize(&ysw_vs_synth->config);

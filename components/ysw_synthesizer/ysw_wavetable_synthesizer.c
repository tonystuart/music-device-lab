...
static SemaphoreHandle_t synthesizer_semaphore;
...
{
    xSemaphoreTake(synthesizer_semaphore, portMAX_DELAY);
}

static void leave_critical_section()
{
    xSemaphoreGive(synthesizer_semaphore);
}
...
static void run_synthesizer(void* parameters)
{
    ESP_LOGD(TAG, "run_synthesizer core=%d", xPortGetCoreID());
...
void create_synthesizer_task(uint8_t dac_left_gpio, uint8_t dac_right_gpio)
{
    ESP_LOGD(TAG, "create_synthesizer_task: entered");
    ...
    synthesizer_semaphore = xSemaphoreCreateBinary();
    xSemaphoreGive(synthesizer_semaphore);

    ysw_task_create_standard(TAG, run_synthesizer, NULL, 0);

    ESP_LOGD(TAG, "create_synthesizer_task: returning");
}
...
{
    ESP_LOGD(TAG, "run_wt_synth core=%d", xPortGetCoreID());
    for (;;) {
...
{
    create_synthesizer_task(dac_left_gpio, dac_right_gpio);
    ysw_task_create_standard(TAG, run_wt_synth, &input_queue, sizeof(ysw_synthesizer_message_t));
    return input_queue;
}
